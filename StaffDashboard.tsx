...
import { calcCleaningScore, scoreColor, computeRelativeGrades, computeAbsoluteGrade, gradeColor } from "@/lib/scoring";
...
export default function StaffDashboard() {
  ...
  const { data: rawCleaningRecords = [], isLoading: cleaningLoading } = useCleaningInspections(
    filterBranch ? { branch: filterBranch } : {}
  );
  // "입구" 구역은 "공통"으로 이름이 변경됨. 과거 기록도 동일한 구역으로 취급.
  const cleaningRecords = rawCleaningRecords.map(r => r.zone === '입구' ? { ...r, zone: '공통' } : r);
        ...
        <div className="sticky top-0 z-40 bg-white border-b border-border/50 px-4 md:px-[50px] pt-3 space-y-0">
          ...
          <div className="flex border-b border-border">
            <button
              ...
              className={`relative flex-1 flex items-center justify-center gap-1 pb-3 pt-1 text-sm transition-all whitespace-nowrap border-b-2 -mb-px ${
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1 pb-3 pt-1 text-sm transition-all whitespace-nowrap border-b-2 -mb-px ${
          ...
          (activeTab === 'vm' || activeTab === 'quality') ? (
            <div className="flex-1 overflow-y-auto p-4 md:px-[50px] space-y-3">
              {(() => {
                ...
                const isAbsoluteGrading = activeTab === 'vm';
                const gradeMap = isAbsoluteGrading ? null : computeRelativeGrades(
                  rawRanking.filter(r => r.status === 'scored').map(r => r.avg),
                );
                const getGrade = (avg: number | null): 'A' | 'B' | 'C' | null => {
                  if (avg == null) return null;
                  return isAbsoluteGrading ? computeAbsoluteGrade(avg) : (gradeMap!.get(avg) ?? null);
                };
                ...
                return (
                  <>
                    ...
                    <div className="flex gap-1.5 items-center pb-1">
                      ...
                      <span className="text-[10px] text-muted-foreground ml-1">{isAbsoluteGrading ? '95점↑A / 75점↑B / C' : '상위20%/50% 상대평가'}</span>
                    </div>
                              ...
                              <div className="shrink-0 flex items-center gap-1">
                                {isScored && grade ? (
                                  <>
                                    ...
                                    {isAbsoluteGrading && <span className="text-xs font-bold text-muted-foreground">{avg}점</span>}
                                  </>
                                ) : isPending ? null : (
                    ...
                    >
                      ...
                      <div className="p-5">
                        ...
                        <VMCommentThread
                          ...
                          confirmed={(item as any).commentConfirmed ?? false}
                          isAdmin={false}
                        />
