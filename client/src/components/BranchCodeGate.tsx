...
import { useVerifyBranchCode, useLookupBranchByCode } from "@/hooks/use-branch-code";
    ...
    <div className="flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-xs bg-white rounded-3xl border border-border shadow-lg p-6 space-y-5">
        <div className="text-center space-y-1.5">
          <h2 className="text-lg font-black text-secondary">{branch ? "지점 코드 확인" : "점포 코드를 입력하세요"}</h2>
          <p className="text-sm text-muted-foreground">
            {branch ? "점검을 시작하려면 코드를 입력하세요" : "코드를 입력하면 해당 점포로 바로 이동합니다"}
          </p>
        </div>
        ...
        {error && (
          <p className="text-center text-xs font-bold text-primary -mt-2">
            {branch ? "코드가 올바르지 않습니다" : "일치하는 점포 코드가 없습니다"}
          </p>
        )}
